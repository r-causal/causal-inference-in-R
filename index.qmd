...
1. Understand the assumptions needed for causal inference 
1. Identify the target population for which you want to make inferences 
1. Fit causal models and check their problems 
1. Conduct sensitivity analyses where the techniques we use might be imperfect
...
It's also helpful to realize the native lamda is identical to `function(.x) do_something(.x)`, where `\` is shorthand for `function`.
