...
It's the outcome model where we estimate the causal effect.
For many outcome models, this stage is considerably simpler than the design phase: we fit a regression model in the same way we would normally, but without the confounders.
Instead of including the confounders in the model, we either use a matched dataset or weight the outcome model by the propensity score weights.
There's a hiccup, though: the standard errors that come packages with most regressions toolkits do not account for the design phase.
Thus, they will often produce incorrect uncertainty estimates.
In this chapter, we'll discuss using matched datasets and weighted datasets, as well as a few different ways to calculate standard errors.
...
If we have succeeded in doing so, we can compare the difference in wait times for both groups without an additional covariates, much as we can in randomization.
There are other good reasons to include covariates besides the outcome, including improved precision, heterogenous treatment effects, and so on, but for now we will focus on the simple question at hand: does having Extra Magic Mornings change the posted wait time between 9 A.M. and 10 A.M.?
...
TODO: make this update based on the code.
...
We will use the ATT weights so the effect estimate is comparable to the matching-based analysis above.

Our workflow is identical to the previous way we calculated the propensity score weights, except we use `propensity::wt_ate()`.
