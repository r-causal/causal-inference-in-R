...
We are trying to estimate a causal effect of frequency of software updates on customer satisfaction (measured as a standardized score with a population mean of 0 and standard deviation of 1).
...
Even though there is no relationship between the exposure and outcome, we expect there to be confounding from the open backdoor path between `updates` and `satisfaction` via `customer_type`.
...
ggdag(dag1, use_text = FALSE, use_edges = FALSE) +
  geom_dag_text(ggplot2::aes(label = label), nudge_y = c(-.004, -.004, -.004), color = "black") + 
  geom_dag_edges_arc(curvature = c(0.01, 0)) +
  theme_dag()
```
...
Many of our simulations in this book will skip this step and simulate the observed outcomes directly; however, as we transition into *answering* causal questions, it's helpful to remember what we assumptions we need to meet in order to make inferences.
...
satisfaction1 <- tibble(
  ...
  # (notice `update_frequency` is not in the equation below, only the confounder)
...
```{r}
satisfaction1 |>
  select(update_frequency, customer_type, satisfaction, observed_potential_outcome)
```
...
Of course, we know from the DAG and from the way we simulated the potential outcomes that the two groups are not exchangeable.
...
Let's do a little wrangling to get an estimate of the effect of update frequency on satisfaction within levels of customer type.
...
```{r}
satisfaction_strat_est |>
  # note: we would need to weight this if the confounder groups were not equally sized
...
Some customers overlap well with the company's business hours, while some don't; those who don't have lower satisfaction due to unavailability of customer service during their work hours.
...
ggdag(dag2, use_text = FALSE) +
  geom_dag_text(ggplot2::aes(label = label), nudge_y = c(-.004, .004, -.004, -.004, -.004), color = "black") + 
  theme_dag()
```
...
satisfaction2 <- tibble(
  ...
  customer_service = rbinom(n, 1, prob = business_hours * 0.9 + (1 - business_hours) * 0.2), 
...
Within combinations of `customer_type` and `business_hours`, the update frequency groups are very close to each other.
...
Let's say that instead of the binary confounders, we have one continuous confounder: number of users within the organization, as in @fig-satisfaction-dag3.
...
Organizations with more users get more updates, and they also have slightly lower satisfaction scores.
...
satisfaction3 <- tibble(
  ...
  satisfaction = 70 + -0.02 * num_users
) |>
  mutate(
    update_frequency = factor(update_frequency, labels = c("weekly", "daily"))
  )
...
If we want to still use `group_by` and `summarize()`, we could bin the continuous confounder, for example using it's quintiles, and estimate the causal effect within each bin:
...
Within binned levels of number of users, it seems we get about the right answer.
...
As opposed to binary and categorical confounders, grouping by bins for continuous confounders is not accounting for the variable completely; the coarser the bins, the more residual confounding there will be, and the finer the bins, the closer we'll get to the continuous version (but the fewer number of values we'll have per bin).

The approach that we've been using with `group_by()` and `summarize()` is often called **stratification**.
...
For continuous confounders, this is only partially true, because it's not practical to bin within all levels of the continuous variable.

For simple problems or when you have lots of data, stratification can be quite powerful because you again sometimes avoid model misspecification problems.
However, with many confounders and continuous confounders, we run into the curse of dimensionality quickly, making it impractical because we have too few observations by combination of confounder levels.

## Parametric outcome models

You can think of stratification as conditional means.
A more general extension of conditional means is multivariable linear regression.
When we fit `lm()` with the variables in the form `outcome ~ exposure + confounder1 + confounder2 + ...`, we are fitting what we call an **outcome model**, so called because we are fitting the exposure and confounders with the outcome as the dependent variable.
This is also sometimes called **direct adjustment**, because we are adjusting for the confounders directly in the model.
Let's use `lm()` to calculate the effect in our example with two binary confounders:

```{r}
library(broom)
lm(satisfaction ~ update_frequency + customer_type + business_hours, data = satisfaction2) |> 
  tidy(conf.int = TRUE) |> 
  filter(term == "update_frequencydaily")
```

It also works well for continuous confounders.
We no longer need to bin it to get the correct answer:

```{r}
lm(satisfaction ~ update_frequency + num_users, data = satisfaction3) |> 
  tidy(conf.int = TRUE) |> 
  filter(term == "update_frequencydaily")
```

This generalization doesn't come for free, though: we've now introduced a parametric statistical model to make estimates across the regions of our data that are sparse.
The estimate we get for `satisfaction ~ update_frequency + num_users` gives us exactly the right answer because the statistical model underlying `lm()` matches our simulation perfectly.
For example, the relationship between `satisfaction` and `num_users` is linear, so when we fit this model, we don't suffer from the problem of dimensionality, although linear regression has its own limits in terms of number of rows and number of columns.
...
This requires an understanding of the functional relationship between these variables and the outcome.
We'll also later explore ways to use data-adaptive methods like machine learning to reduce this assumption (@secsec-causal-ml).

<!-- TODO: link appendix on model checks here -->

When we've met the assumptions of the estimator we're using for our model, outcome regression can work very well.
In the case of OLS, if we understand the relationships between the outcome and the variables in the regression, we get a lot of benefits.
OLS is very efficient, statistically (meaning we'll get a small standard error).
We'll also get nominally correct confidence intervals without needing to bootstrap (@sec-appendix-bootstrap).
Scientists and other people who analyze data are also usually familiar with linear regression, making it easier for many to understand what you did to calculate the causal effect.

In fact, you could say that when there is a linear relationship between the outcome and exposure and we meet the causal assumptions laid out in @sec-assump, *correlation is causation*.

So, why don't we always outcome models to calculate causal effects?
First, we may be more confident in modeling the exposure instead of the outcome (as we do in, for instance, inverse probability models).
We'll explore this idea more in @sec-g-comp and @sec-dr.
Second, it can be sometimes challenging to to get the estimate we are targeting with outcome models---an answer to a precise question---something we will probe more deeply in @sec-estimands.

Relatedly, outcome models give us *conditional* coefficients.
In other words, when describing the estimated coefficient, we often say something like "a one-unit change in the exposure results in a `coefficient` change in the outcome *holding all other variables in the model constant*. 
In causal inference, we are often interested in *marginal* effects. 
Mathematically, this means that we want to average the effect of interest across the distribution of factors in a particular population that we are trying to estimate a causal effect for. 
In the case where the outcome is continuous, the effect is linear, and there are no interactions between the exposure effect and other factors about the population, the distinction between an conditional and a marginal effect is largely semantic. 
The estimates will be identical.
...
Why?
We are ultimately trying to determine whether we should suggest the exposure to the target population, so we want to know *on average* whether it will be beneficial.
...
This means the effect of changing the update frequency is heterogeneous, depending on customer type.
Whether it's beneficial increase the update frequency to daily for everyone depends on the distribution of premium vs. free customers.

-   If **50% of your customers are Premium** and **50% are Free**, the average effect of switching to daily updates would be:

    $(0.5 * 5) + (0.5 * -5) = 0$

-   If **100% of your customers are Premium**, the average effect would be:

    $(1 * 5) + (0 * -5) = 5$

-   If **100% of your customers are Free**, the average effect would be:
...
We may, however, be able to use another method like instrumental variable analysis, regression discontinuity, or difference-in-differences (@sec-iv-friends and @sec-did; we'll also summarize these methods below).
The second is when we have time-varying exposures and confounding.
Linear regression is not able to estimate these types of effects without bias, so we'll need methods like inverse probability weighting or g-computation to calculate it correctly.
Through most of the book, we'll analyze simple pre-post data: we have baseline data, an exposure that happens at a single outpoint, and an outcome that occurs after exposure.
In @sec-longitidinal and other chapters, we'll cover more complex questions and data.
...
For the rest of the book, however, we will focus on other causal methods, which allow us more flexibility to answer the questions we want to answer.

-   Unconfoundedness methods
    -   Inverse probability weighting (propensity score weighting):
    -   Matching (propensity score matching and other methods):
    -   G-computation (also called standardization or marginal effects):
    -   Doubly-robust methods: TMLE and augmented propensity scores. ML

While more of the book focuses on unconfoundedness methods, we later cover methods that make other assumptions (@sec-iv-friends and @sec-did).
...
-   Synthetic controls: A weighted combination of untreated units can closely approximate what the treated unit’s outcome would have been without the treatment. Synthetic controls are closely related to difference-in-differences.
...
Let's consider a variation of @fig-satisfaction-dag2 where frequency of updates is randomized to each customer; customer type and business hours are still causes of customer satisfaction. In other words, they cause the outcome but not the exposure. To get a valid effect, can use an unadjusted regression model or a simple differences in means. As we discussed in @sec-dags, however, including causes of the outcome that are not causes of the exposure can improve statistical precision of an estimate. Let's look at three approaches: an unadjusted OLS outcome model, an adjusted OLS outcome model (direct adjustment), and an inverse probability weighted model. In @fig-panel, all three methods give us an unbiased effect.
The effect from the propensity score is a marginal effect, while the effect from the outcome model is a conditional effect.
Because of the way we've simulated the data, the two types of effects are identical.
The unadjusted method, however, has a wider confidence interval and, relatedly, a larger standard error. Both the direct adjustment method and inverse probability weighting have smaller standard errors and thus narrower confidence intervals. 
...
plot_estimates <- function(d) {
  unadj_model <- lm(satisfaction ~ update_frequency, data = d) |>
    ...
    dplyr::mutate(term = ifelse(term == "update_frequencydaily", "update_frequency", term)) |> 
    dplyr::filter(term == "update_frequency") |>
    dplyr::mutate(model = "unadjusted")

  adj_model <- lm(satisfaction ~ update_frequency + business_hours + customer_type, data = d) |>
    broom::tidy(conf.int = TRUE) |>
    dplyr::mutate(term = ifelse(term == "update_frequencydaily", "update_frequency", term)) |> 
    dplyr::filter(term == "update_frequency") |>
...
For inverse probability weighting, we do this by accounting for chance imbalances in the treatment groups across variables that relate to the outcome.

Causal methods can also help us address some of the violations of causal assumptions we saw in real-life randomized trials in @sec-designs. We'll explore how these methods can help us address non-adherence and loss-to-follow up, two common sources of bias in randomized trials, in @sec-longitudinal and @sec-iv-friends.

## Entering the design phase

Now, let's turn our attention to an example with real data. 
We're going to start with propensity score methods like matching and inverse-probability weighting because they have a particular property: they allow us to model the relationship between the exposure and confounders without peaking at the relationship between the exposure and the outcome. 
This is sometimes called *the design phase* because it is philosophically similar to designing a randomized trial, closely related to the idea of emulating a target trial.
