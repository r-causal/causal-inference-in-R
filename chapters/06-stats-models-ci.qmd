...
satisfaction1 |>
  select(
    update_frequency, customer_type, 
    satisfaction, observed_potential_outcome
  )
```
...
#| code-fold: true
#| warning: false
#| fig-cap: "A causal diagram of the relationship between frequency of software updates and customer satisfaction. Frequency of updates does not cause customer satisfaction, and the relationship is confounded by their mutual causes, customer type and business hours. The effect of business hours on customer satisfaction is entirely mediated by availability of customer service."
dag2 <- dagify(
...
Relatedly, when we have a binary outcome, it may make sense to choose one over other based on number of events, e.g., it can be more statistically efficient to use a propensity score method if the outcome is rare but the exposure is not.
...
-   *Unconfoundedness methods*
    -   **Inverse probability weighting** (propensity score weighting): Using a propensity score (predicted probability of treatment), we reweight units to create a psuedo-population where exchangeability holds. Extends to time-varying treatments.
