...
Relatedly, when we have a binary outcome, it may make sense to choose one other the other based on number of events, e.g., it can be more statistically efficient to use a propensity score method if the outcome is rare but the exposure is not.
...
In other words, when describing the estimated coefficient, we often say something like "a one-unit change in the exposure results in a `coefficient` change in the outcome *holding all other variables in the model constant*. In causal inference, we are often interested in *marginal* effects. Mathematically, this means that we want to average the effect of interest across the distribution of factors in a particular population that we are trying to estimate a causal effect for. In the case where the outcome is continuous, the effect is linear, and there are no interactions between the exposure effect and other factors about the population, the distinction between an conditional and a marginal effect is largely semantic. The estimates will be identical.
...
Let's consider a variation of @fig-satisfaction-dag2 where frequency of updates is randomized to each customer; customer type and business hours are still causes of customer satisfaction.
In other words, they cause the outcome but not the exposure.
To get a valid effect, can use an unadjusted regression model or a simple differences in means.
As we discussed in @sec-dags, however, including causes of the outcome that are not causes of the exposure can improve statistical precision of an estimate.
...
Both the direct adjustment method and inverse probability weighting have smaller standard errors and thus narrower confidence intervals.
