...
dagify(
  ...
  geom_dag_edges_arc(curvature = c(rep(0, 5), rep(.2, 3), 0)) +
  geom_dag_point() +
...
denominators <- denominator_model |>
  ...
  select(date, denominator, .fitted)
```
...
If the mean is far from 1, we may have issues with model misspecification or positivity violations [@hernán2021].
...
#| fig.cap: >
#|   A scatter of the stabilized inverse probability weights for posted waiting time vs. posted waiting times. Days with more values of `avg_spostmin` farther from the mean appear to be downweighted, with a few exceptions. The most unusual weight is for June 23, 2018.
