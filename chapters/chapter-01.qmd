...
Translating this idea into asking good causal questions, we can map the following terms that you will see throughout this book to these diagrams: *exposure* (the cause), *outcome* (the effect), *eligibility criteria* (for whom?), *time zero* (when did the participants begin to be followed?), *follwed-up*, (who can we estimate an outcome effect for?) and *follow up period* (when?).
...
data <- data.frame(labels = c("exposure", "outcome", "eligibility criteria", "time zero", "followed-up", "follow-up time"),
