...
In general, background knowledge (not prediction or associational properties) should drive variable selection for causal models @robinsImpossible; we'll discuss this process in detail in [Chapter -@sec-dags].
...
Consider a model that is causally unbiased for an exposure but only includes variables related to the outcome *and* the exposure. 
In other words, this is a model that provides us the right answer for the exposure of interest, but doesn't include other predictors of the outcome (which can sometimes be a good idea, as discussed in @sec-data-causal).
...
The causal claim here could be *smoking causes lung cancer*.
...
Do the answer match the question?
Not quite.
