...
It also helps us be certain that the data structure we're using matches the question we're trying to answer, as we'll see in [Chapter -@sec-data-causal-qs].
...
Let's stick with the COVID-19 example [^chapter-01-1].
...
We aren't always able to assess against the truth, but it's almost always required for fitting the initial model [^chapter-01-2].
...
Causal inference always has a relationship to explaination, but we can accurately estimate the effect of one thing on another without understanding how it happens.
...
He was right: contaminated water was a mechansim for cholera transmission.
