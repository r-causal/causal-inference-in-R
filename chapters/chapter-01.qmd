...
One important detail of descriptive anlysis is that confounding bias, one of the chief concerns of this book, is undefined. That's because confounding is a causal concern. Without a causal question, descriptive 


#### Relationship to causal inference

Humans are built to see patterns; we can't help but see them, and this is particularly true in data analysis. This is a really helpful feature of our brains, but it can also lead down a slippery slope of inference when we're not working with data or methods that can allow us to do that. The single biggest thing to be cautious of is making the leap from description to causation (implied or explicit). 
...
Finally, as we'll see in TODO: CHAPTER REF, there are certain circumstances where we can make causal inferences with basic statics. Be cautious about the distinction between the causal question and the descriptive component here, as well. Just because we're using the same calculation (e.g., a difference in means) doesn't mean that all descriptions you can generate are causal in nature.
...
Predictive modeling generally uses a different workflow than the one we'll present in this book for causal modeling. Since the goal of prediction is usually related to making predictions on new data, the workflow of this type of modeling focuses on maximizing predictive accuracy while retaining generalization to new data, sometimes called the bias-variance trade-off. In practice, this means splitting your data into training sets (the part of the data you build your model on) and test sets (the part of the data you assess your model with, a proxy for how it would perform on new data). Usually, data scientists use cross-validation or other sampling techniques to further reduce the risk of overfitting your model to the training set. 
...
The key measure of validity in prediction modeling is predictive accuracy, which can be measured a number of ways. A convenient detail about predictive modeling is that we can often assess if we're right.       
