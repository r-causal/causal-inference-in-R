...
Prediction is the most popular topic in data science, in large part thanks to applications of machine learning in industry. Prediction, of course, has a long history in statistics, and many models popular today have been used for decades in and outside of academia.

Let's stick with the COVID-19 example[^2]. In 2021, researchers published the ISARIC 4C Deterioration model, a clinical prognostic model for predicting serious adverse outcomes for acute COVID-19. The authors included a descriptive analysis to understand the population from which this model was developed, particularly the distribution of the outcome and candidate predictors. One useful aspect of this model is that it uses items commonly measured on day 1 of COVID-related hospitalization. The authors built this model using cross-validation by region and the tested the model on data from a hold-out region. The final model included 11 items and a description of their model attributes, relation with the outcome, and so on. Importantly, the authors used clinical domain knowledge to select candidate variables but did not fall to the temptation of interpreting the the model coefficients as causal. Without question, some of the predictive value of this model stems from the causal structure of the variables, but this model had a different goal entirely and stuck to it. 

[2]: One obvious case in this area is to predict cases, but modeling infectious diseases is a bit complex and usually use modeling techniques that differ from those common in other prediction models. 
...
We'll see a lot of examples of causal inference in this book, but let's continue on with an example related to COVID-19. As the pandemic continued on and tools like vaccines and anti-viral treatments became available, policies like universal masking also began to change. In February 2022, the US state of Massachusetts rescinded a statewide policy that required universal masking in public schools. In the greater Boston area, some school districts continued the policy while others discontinued it; those that discontinued it also did so at different times over the following weeks after the policy change. This allowed researchers to take advantage of the differences in district policies over this period to study the impact of universal masking on COVID-19 cases. The researches included a descriptive analysis of the school districts to understand the distribution of factors related to COVID-19 and other determinants of health. To estimate the effect of universal masking on cases, the authors used a technique common in policy-related causal inference called difference-in-differences to estimate this effect. The design that they used alleviates some of the difficult assumptions needed for causal inference, but they also wisely controlled for potential confounders despite that advantage. The authors found that districts that continued masking saw a drastically lower case load than those who didn't; their analysis concluded that almost 12,000 additional cases occurred due to the policy change, nearly 30% of the cases in the districts during the 15-week period of the study.
...
Unfortunately, this is not always the case; causal effects needn't predict particularly well and good predictors needn't be causal.
...
When effects are small, data are noisy, or TODO: ADD OTHER REASONS, then it might make sense to use a biased model like penalized regression.
...
Descriptively, ice cream sales and crime are related, but this relationship is confounded by weather, e.g. both ice cream sales and crime increases when it's warmer.
...
A closely related idea is the *Table Two Fallacy*, so-called because in health research papers, descriptive analyses are often presented in Table 1 and regression models are often presented in Table 2.
