...
In predictive modeling, people often have to prevent overfitting their models to chance patterns in the data.
When a model captures those chance patterns too much, it doesn't predict as well on other data sets.
...
The correct causal model (the functional form that matches the data-generating mechanism), as with the correct predictive model, cannot be overfit.

There's some nuance to this answer, though.
Overfitting in causal inference and prediction are different; we're not applying the causal estimate to another dataset (the closest to that is transportability and generalizibility, an issue we'll discuss in [Chapter -@sec-evidence]).
It remains true that causal model needn't predict particularly well to be unbiased.

In prediction modeling, people often use the bias-variance trade off as a way to improve out-of-data predictions.
In short, some bias for the sample is introduced to improve the variance of model fits to make better predictions out of sample.
We have to be careful here, though: the word bias here refers to the discrepancy between the model estimates and the true value of the dependent variable *in the dataset*.
...
If we apply the bias-variance trade off to causal models, we're introducing statistical bias in an attempt to reduce causal bias.
Another subtlety is the overfitting can inflate the standard error of the estimate in sample, which is not the same variance as the bias-variance trade off [@schuster2016].
...
In practice, cross-validation, a technique to reduce overfitting, is often used with causal models that make use of machine learning, as we'll discuss in [Chapter -@sec-causal-ml].
:::
