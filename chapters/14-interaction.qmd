...
**Effect modification** occurs when the effect of one exposure (X) on an outcome (Y) varies across strata of a third variable (Q).
In effect modification, we're interested in how the effect of X changes depending on Q.
...
Effect modification is asymmetric: it focuses on the effect of X, not on the effect of Q.
Importantly, Q must be a pre-treatment variable which cannot be affected by X [@hernan2021].

**Interaction**, on the other hand, concerns whether the joint effect of two exposures (X and Q) together differs from what we'd expect based on their individual effects.
In interaction, we consider interventions on both X and Q.
Interaction is symmetric: both X and Q are intervention variables of equal interest, and the definition gives them equal status [@hernan2021].
...
Let $Y(x)$ denote the counterfactual outcome $Y$ under an intervention to set exposure $X$ to level $x$.
Let $Y(x,q)$ denote the counterfactual outcome $Y$ under interventions to set both X and Q to levels x and q, respectively.

**Effect modification** on the causal risk difference scale occurs when:
$$E[Y(1) | Q = q_1] - E[Y(0)| Q = q_1] \neq E[Y(1) | Q = q_0] - E[Y(0) | Q = q_0]$$

Notice that we're conditioning on Q (looking at $Y(x)$ given Q), not intervening on it.
...
The key question we are asking is "Does the effect of X vary across levels of Q?"
...
$$E[Y(1, q_1)] - E[Y(0, q_1)] \neq E[Y(1,q_0)] - E[Y(0, q_0)]$$

Here, we're intervening on both X and Q (looking at $Y(xq)$).
This definition asks: does the effect of X vary depending on what level we set Q to?

Because interaction is symmetric, this definition is equivalent to asking whether the effect of Q varies depending on what level we set X to:
$$E[Y(1, q_1)] - E[Y(1, q_0)] \neq E[Y(0, q_1)] - E[Y(0, q_0)]$$

The two formulations are mathematically equivalent and show that X and Q have equal status in the definition of interaction [@hernan2021].
...
Effect modification asks about variation in the effect of X across observed levels of Q.
Interaction asks about variation in the effect of E across intervened-upon levels of Q (or equivalently, variation in the effect of Q across intervened-upon levels of X).
...
Here we demonstrate three approaches using a simple example where X (treatment) and Q (another exposure) may interact in their effect on D (outcome), with Z as a confounder.
...
This DAG shows that X, Q, and Z all affect Y, but it doesn't explicitly indicate whether X and Q interact.
...
Here, X is a drug, Y is hypertension, Z is a person's genotype, and Q is hair color.
Hair color serves as a proxy for a genotype that drives risk of hypertension.

```{r}
#| label: fig-effect-mod-no-interaction
#| fig-cap: "A DAG showing effect modification by Q (hair color) of the effect of X (drug) on Y (hypertension) without interaction between X and Q. Hair color Q is a proxy for genotype Z, which modifies the effect of X on Y."
#| fig-width: 5
#| fig-height: 4
...
In this DAG, Q (hair color) can serve as an effect modifier for the effect of X (drug) on Y (hypertension) because Q is a proxy for Z (genotype), which actually modifies the effect.
We might observe that $E[Y(x) | Q = q_1] - E[Y(x) | Q = q_0] \neq E[Y(x) | Q = q_0] - E[Y(x) | Q = q_0]$, meaning the effect of the drug varies across hair color groups.

However, there is no interaction between the effects of X and Q on Y.
...
We have $E[Y(1, q)] - E[Y(0,q)] = E[Y(1)] - E[Y(0)]$ for all values of q.
...
X is a weight-loss drug given in a randomized trial, Y is weight at the end of the trial, Z is sugar intake, and Q is exercise level.

```{r}
#| label: fig-interaction-no-effect-mod
#| fig-cap: "A DAG showing potential interaction between the effects of X (weight-loss drug) and Q (exercise) on Y (weight) without effect modification by Q of the effect of X on Y. Sugar intake Z affects both exercise Q and weight Y."
#| fig-width: 5
#| fig-height: 4
...
Suppose the effects of the weight-loss drug X and exercise Q interact such that $E[Y(x_1,q_1)] - E[Y(x_0,q_1)] \neq E[Y(x_1,q_0)] - E[Y(x_0, q_0)]$.
...
However, Q might not be an effect modifier for the effect of X on Y.
Sugar intake Z affects both exercise Q and weight Y.
...
These cancellations can occur regardless of whether a participant receives the drug X.

It's therefore possible that we observe $E[Y(x_1) | Q = q_1] - E[Y(x_0) | Q = q_1] = E[Y(x_1) | Q = q_0] - E[Y(x_0) | Q = q_0]$, meaning Q is not an effect modifier, even though there is causal interaction between the effects of X and Q when we intervene on both.

The key insight is that conditioning on Q (as in effect modification) captures information about common causes of Q and Y (like Z), while intervening on Q (as in interaction) does not.
...
# Simulate data where:
# - Z (sugar intake) affects both Q (exercise) and Y (weight)
# - X (drug) and Q interact in their effect on Y
# - But conditioning on Q masks the interaction due to confounding by Z
# We set up the parameters so that when we condition on Q without controlling for Z,
...
sim_data <- tibble(
  ...
  Q = rbinom(n, 1, plogis(-0.5 + 1.2 * Z)),
  # Drug assignment (randomized, so independent of Z and Q)
  X = rbinom(n, 1, 0.5),
  # Weight outcome with interaction between X and Q
  # But also affected by Z (common cause of Q and Y)
  # The coefficients are carefully chosen so that conditioning on Q without Z
  # creates confounding that attenuates the interaction
  # True interaction: -4, but Z's effect on Y creates bias when we condition on Q
  Y = 70 - 2 * X - 1 * Q - 4 * X * Q + 3.5 * Z + rnorm(n, 0, 2)
)

# Check: Is there interaction when we intervene on both X and Q?
# We can assess this by controlling for Z (the confounder)
mod_interaction <- lm(Y ~ X + Q + X:Q + Z, data = sim_data)
tidy(mod_interaction) |>
  filter(str_detect(term, "X|Q"))
```

The interaction term `X:Q` is `r round(coef(mod_interaction)["X:Q"], 2)`, indicating there is interaction between X and Q when we control for Z.
