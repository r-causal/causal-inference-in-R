...
Often we are interested in assessing how some *exposure* (or treatment) impacts an outcome.
...
As defined in the previous chapter, an exposure in the context of this book is an often modifiable event or condition that occurs prior to the outcome.
...
This is done by fitting a logistic regression model predicting the exposure using known covariates.
...
In the code above, the first `geom_histogram()` call creates the top histogram by subsetting the `df` data frame to only include those in the exposure group, where `extra_magic_morning == 1`.
