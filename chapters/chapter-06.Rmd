...
At it's heart, the workflow is the same: we fit a model where the exposure is the outcome then use that model to weight a second outcome model.
...
We're assuming that are primary confounders are the time the park closes, historic high temperatures, whether or not the ride has extra magic morning hours, and the ticket season.
Similar to above, each of the confounders precede the exposure and outcome, and (by definition) the exposure precedes the outcome.
...
Fitting the model is similar to the binary exposure case, but we need to use linear regression, as the posted time is a continuous variable.
...
When some observations have extreme weights, the propensities are *destabalized.* We can stabilize them using the marginal distribution of the exposure.
...
```{r}
ggplot(great_data_name_wts, aes(swts)) +
  geom_density(col = "#E69F00", fill = "#E69F0095") + 
  scale_x_log10() + 
  theme_minimal(base_size = 20) + 
...
The best way to decide what variables to include in your propensity score model is to look at your DAG and include at least a minimal adjustment set of confounders.
Of course, sometimes, important variables are missing or measured with error.
Often there is more than one theoretical adjustment set than can debias your estimate; it may be that one of minimal adjustment sets is measured well in your data set and another is not.
...
Specifying a DAG accurately greatly improves our ability to add the correct variables to our models.
However, confounders are not the only important type of variable to consider.
...
Luckily, in practice, it seems that this bias is relatively negligible, especially compare to the risk of confounding bias.
...
There are some circumstances that make this difficult, however: some colliders are inherently stratified on by the design of the study or the nature of follow-up.
For example, loss-to-follow-up is a common source of collider-stratification bias; we'll discuss this further in Chapter XX.

There's another situation where a variable is both a confounder and a collider, as in the case of so-called butterfly bias:

```{r}
ggdag(butterfly_bias())
```

If we have all the variables measured well, we can avoid adjusting for `m` by adjusting for either `a` or `b`.
However, if we don't have those variables, what should we do?
Adjusting for `m` opens a biasing path way that we cannot block through `a` and `b` (collider stratification bias), but `m` is also a confounder for `x` and `y`.
