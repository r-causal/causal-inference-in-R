...
The odds ratio for those with `covariate = 0` is `r conditional_or_0`. For those with `covariate = 1`, it's `r conditional_or_1`. 

The risk ratio...

(weighted average)

So, although the subgroup risk ratios differ, you can still calculate the marginal risk ratio.

* non-collapsibility is not bias. some describe as omitted variable bias

::: {.callout-note}
## The linear probability model 
:::

* multiplicative vs additive data model
* include as many causes of outcome as is reasonable
* present all effects rather than picking just one (among targetted subgroup, e.g. the treated)
