...
Preparing data and doing exploratory analysis is, in many respects, the same for causal inference as for description and prediction.
What differs for causal inference is the way we'll link the process to our causal question and target trial emulation.
We'll also use this opportunity to better understand how well our data might meet the causal assumptions, although, as we've seen in @sec-quartets, data can never tell us whether we're right or not.

Let's now take a look at the data and causal question we'll examine for the next several chapters.
...
Until 2019, every year totals up to the full year (2016 was a leap year). In 2020 and 2021, of course, the park was limited by the COVID-19 pandemic, and so there are fewer days available.
...
For each `park_date`, we have a number of reports of wait times, both posted wait times (`wait_minutes_posted`, scraped from the times posted on Disney's website) and actual wait times (`wait_minutes_actual`, reported by individuals who actually waited in line). Each row is a record of either posted or actual wait minutes at a given `wait_datetime`, with the other value being `NA` for that row.
...
Here's our causal question:

**Is there a relationship between whether there were Extra Magic Hours in the morning at Magic Kingdom and the average posted wait time for the Seven Dwarfs Mine Train the same day between 9am and 10am in 2018?**

Let's begin by diagramming this causal question (@fig-seven-diag).

```{r}
#| code-fold: true
#| fig-cap: "Diagram of the causal question \"Is there a relationship between whether there were Extra Magic Hours in the morning at Magic Kingdom and the average posted wait time for the Seven Dwarfs Mine Train the same day between 9am and 10am in 2018?\""
...
We are interested in examining if between whether there were Extra Magic Hours in the morning ("Extra Magic Morning") causes a change in the average posted wait time for the Seven Dwarfs Mine Train on the same day between 9 am and 10 am.

@fig-dag-magic is a proposed DAG for this question.
We assume that Extra Magic Morning is decided based on the time the park closed, the ticket season, and the historic high temperature.
Likewise, these three variables are also causes of the average posted wait time.
Of course, this is a greatly simplified DAG.
Moreover, *someone* at Disney knew the assignment mechanism for decided Extra Magic Hours for 2018, so if we were working there, we'd want to find out as much as we could about that process.
For the example's sake, though, we'll keep it simple.
...
You can think of the steps of a protocol as actions to take. In a randomized trial, many of these actions are part of the trial design and data collection. In a target trial emulation, we often need to apply those actions ourselves to the data we are preparing to answer causal questions.   @tbl-dplyr presents the type of actions (here, functions in the tidyverse) we might need to take. 
...
To answer our causal question, we need to manipulate both the `seven_dwarfs_train` dataset and the `parks_metadata_raw` dataset.
...
Let's take a look at this dataset. The range of the dates looks reasonable, as do the posted wait times. The actual wait times, though, have a minimum of `r min(seven_dwarfs_train$wait_minutes_actual, na.rm = TRUE)`! 
...
The distribution of the wait times is quite wide, with actual times appearing smaller on average than posted times. 
...
The posted times are also more jagged. They seem to be rounded to the 5 minutes.
...
::: {.callout-note}
Doing this kind of data checking is important to understand the limits and potential issues in the data. R has many great tools for quickly summarizing data sets, such as `skimr::skim()` and `pointblank::scan_data()`. We also recommend using a data validation tool like pointblank to write down and test your expectations about how the data should be.
:::

We need this dataset to calculate our outcome, defined as the average posted wait time between 9am and 10am.
Our eligibility criteria states that we need to restrict our analysis to days in 2018.
...
seven_dwarfs_9 <- seven_dwarfs_train |>
  ...
  # replace NaN with NA
  # this occurs when there is mean of a
...
Examining @fig-dag-magic, we see that we three open backdoor paths. We can close them with the three confounders on each path: the ticket season, the time the park closed, and the historic high temperature.
...
It appears not all ticket season types happen every month. There weren't any peak tickets in August or September, and there weren't any value tickets in June, July, or December.
...
There were many more regular ticket days in the summer and more peak ticket days in March, May, and December.
...
Through much of the year, the Magic Kingdom was open until 10 PM, 11 PM, or midnight, although there was considerable variety, including one day that ended at 4:30 PM.
...
```
Disney World is in Florida, so it never gets particularly cold but gets quite hot in the summer.
...
Let's now join the confounder and exposure data (in `parks_metadata`) to the outcome data (in `seven_dwarfs_9`) to make a single analytic dataset. In this case, we have a simple, 1:1 match, so we can use a left join. 

::: {.callout-note}
Joining is a nuanced data wrangling topic but often essential for creating an analytic dataset. We recommend *R for Data Science* for a thorough discussion of joins.
:::

Notably, we don't have a match for every day. 2018 had 365 days, which is how many rows we have in `parks_metadata`. `seven_dwarfs_9`, though, only has 362.
...
At any rate, let's join the datasets together by date. Now we have a single analytic dataset to do our causal analysis on.
...
Let's take a bird's eye view of the data set by creating a descriptive table. R has many tools for doing so; we are going to use the `tbl_summary()` function from the `{gtsummary}` package.
...
Among close times, we see that the most common times were 22:00 and 23:00. There are also several sparse or empty cells, e.g., close times where we may have positivity violations. 
...
seven_dwarfs_9 |>
  mutate(park_extra_magic_morning = factor(park_extra_magic_morning, labels = c("No Extra Magic Hours", "Extra Magic Hours"))) |>
  tbl_summary(
...
```
It turns out the missing observations for the posted wait times weren't limited to the dates with no observations. Some had records that were empty. For instance, January 24th had 9 records, but both types of wait times were missing.
...
All said and done, though, we're only missing posted wait times for 8 days, about 2% of the year.
This is not too bad.
...
Since we're not using this outcome yet, though, we'll set that aside, too.

## Exploring causal assumptions

As we've seen in @sec-assump and @sec-quartets, data can't solve the problem of the unverifiable assumptions we need to make to do causal inference. However, it can still provide information.

Exchangeability is a tough assumption to check. In many cases, a confounder will be associated with both the exposure and the outcome. However, the relationships between the confounder and these two variables may themselves be confounded. We'll save data checks for exchangeability for @sec-eval-ps-model, where we will have better tools to probe this assumption.

One way we can check for consistency is to use data for multiple versions of treatment. For instance, if our question had instead been about "Extra Magic Hours" rather than "Extra Magic Hours in the morning", we might have a consistency violation. Extra Magic Hours also happen in the evening, and it's entirely plausible that that would have a different effect than those in the morning. We could explore the data by instead separating the two types of Extra Magic Hours into separate exposures. There are also harder to measure ways that Extra Magic Mornings might different from one another; we might not be able to check these without a lot of detective work. Since we've already clarified that we're using Extra Magic Mornings, we won't probe this assumption further.
...
The only way we can tell the difference, though, between stochastic and structural positivity is via background knowledge.
...
Both exposure levels span the majority of the covariate space, but none of days where the Magic Kingdom closed at 16:30 and 21:00 had Extra Magic Mornings. 
...
As we know, there was only one day that ended at 16:30, but there were 28 that ended at 21:00, none of which had Extra Magic Mornings.
...
Examining @fig-temp, it does look like there are very few days in the exposed group with maximum temperatures less than 60 degrees. 
...
If we found this to be particularly troubling given our understanding of the problem, we could consider changing our causal question to instead restrict the analysis to warmer days.
This would also restrict which days we could draw conclusions about. 
...
prop_exposed <- seven_dwarfs_9 |>
  ...
  summarize(prop_exposed = mean(park_extra_magic_morning), .groups = "drop") |>
  complete(
    park_close_bin,
...
The only days with cold temperatures in Florida that would also be considered a "peak" time to visit Walt Disney World would be over Christmas and New Years.
During this time, there historically were always extra magic hours. 

We also have nine combinations which were never exposed.
...
Are these chance occurrences or structural ineligibility for these days to be Extra Magic Mornings? If they are chance occurrences, can we make statistical assumptions that will allows us to validly extrapolate across these empty regions of covariate space in our data? In either case, should we change the question we're asking, either by eligibility or estimand? For now, we are going to continue without making changes to the research question or target trial emulation, but we will keep these observations in mind and explore other options in future sections.
