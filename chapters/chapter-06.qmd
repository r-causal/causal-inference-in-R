...
The *positivity* assumption requires that within each level and combination of the study variables used to achieve exchangeability, there are exposed and unexposed subjects (@sec-assump). We can explore this by visualizing the distribution of each of our proposed confounders stratified by the exposure. 

### Single variable checks for positivity violations

@fig-close shows the distribution of Magic Kingdom park closing time by whether the date had extra magic hours in the morning. There is not clear evidence of a lack of positivity here as both exposure levels span the majority of the covariate space.
...
Finally, let's look at the distribution of ticket season by whether there were extra magic hours in the morning. Examining @fig-ticket, we do not see any positivity violations.
