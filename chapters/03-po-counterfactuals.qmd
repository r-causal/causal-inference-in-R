...
This led him to leave behind his criminal past and launch a successful music, film, and television career.
...
One is their happiness if they ate chocolate ice cream (defined as `y_chocolate` in the code below), and one is their happiness if they ate vanilla ice cream (defined as `y_vanilla` in the code below).
...
Most of the assumptions we need to make for causal inference are to make an apples-to-apples comparison: We want to make sure we're comparing similar individuals who would serve as reasonable proxies for each other's counterfactuals.
...
As our sample size increases, we'd achieve better exchangeability, but this quickly becomes difficult without a good statistical model.
:::

### Positivity

The positivity assumption states that every individual has a non-zero probability of receiving each level of exposure.
Mathematically, this means that $P(X = x) > 0 \quad \text{for all } x$ In other words, we assume that there is no one for whom one or more levels of exposure are impossible.
...
A nuance of positivity is that it needs to hold within levels of all covariates required for exchangeability: $P(X = x \mid Z = z) > 0 \quad \text{for all } x \text{ and } z$ Even if the flavors vary, we also need variability within levels of `prefer_chocolate`.
...
If you think this issue through, though, you'll see that many times, this assumption is violated nearly infinitely for any given exposure.
Let's consider two common cases:

-   **Poorly-defined exposure**: For each exposure value, there is a difference between subjects when delivering that exposure. Put another way, multiple treatment versions exist; we assume a *well-defined exposure*.
-   **Interference**: The outcome (technically all potential outcomes, regardless of whether they are observed) for any subject depends on another subject's exposure; we assume *no interference*.
...
Of course, Eating spoiled ice cream makes you miserable, regardless of how you feel about chocolate ice cream in general.
...
We can imagine other ways in which slight variations in the treatment occur: a high-quality brand of vanilla ice cream and a low-quality brand of vanilla ice cream are both categorized as "vanilla." One person eats in the morning and another in the afternoon.
...
Now, we can group it by its actual exposure.
...
However, it can also occur in a variety of other settings, including via social networks, policy interventions, the geographic proximity of treated units, and so on.
...
Here, each individual, unique *ID*, is considered a unit, and there is interference between units (i.e., partners).
Suppose we consider each *partner* as a unit and randomize the partners rather than the individuals.
...
For example, if we want to know what would happen if both people ate chocolate ice cream versus if both people ate vanilla ice cream, we would want to randomly assign both partners to either chocolate or vanilla, as seen below.
...
When the assumptions for a causal method are not met, we are not able to use the observed data to simulate potential outcomes to compare.
We can address many of the potential violations, however, through careful design of studies, whether prior to collecting data or working with "found" observational data.
...
Similarly, positivity is guaranteed by the randomization process so long as no one has a deterministic exposure; we know the probabilities we use to assign exposures.
Randomized trials also help, but do not guarantee, consistency, particularly for well-defined exposures.
An exposure needs to be defined in order to administer it.
Interference, however, can still happen.
For instance, if we randomize some people to receive a vaccine for a communicable disease, their receiving it could lower the chance of those around them contracting the infectious disease because it changes the probability of exposure.
This requires additional considerations in the design of the trial, e.g., cluster randomization or separation of units, as the examples above.
...
In real randomized trials, things can go wrong.
...
Attempts to prevent interference can also fail, e.g., randomizing on the wrong unit or units interacting with one other in an unexpected way.

Non-randomized (observational) studies have none of these guarantees.
...
@tbl-assump-solved summarizes how well ideal randomized trials, realistic randomized trials, and observational studies each meets the exchangeability, positivity, and consistency.
...
We might also just be able to get a preliminary answer sooner with observational data while randomized trials play out.
...
In @tbl-protocol we map each of these elements to the corresponding assumption that it can address.
...
-   **Eligibility criteria**: Eligibility criteria is particularly useful for preventing positivity violations (by excluding those who can never have one of the exposures), but it's also useful for consistency and exchangeability. You can, for instance, exclude people who are likely to effect each other's outcomes, or only include people without extreme characteristics.
...
-   **Assignment procedures**: In randomized assignment, exchangeability and positivity are met. Understanding the way an exposure come to happen also improves exchangeability and positivity in non-randomized studies. As we've seen, we can also use assignment procedures to prevent interference, and likewise it can help improve a well-defined exposure since the assignment mechanism needs to be clear.
-   **Follow-up period**: We'll discuss the issues that defining a follow-up period in more detail in @sec-longitudinal, but it helps with exchangeability by making sure the follow-up time is comparable for all exposure levels. It's also closely related to consistency because a well-defined exposure may require a time element.
-   **Outcome definition**: Defining the outcome precisely helps identify prognostic factors, which in an randomized trial improve precision but in an observatioal study can help improve exchangeability.
...
-   **Analysis plan**: In observational studies, a clear analysis plan improves all aspects of meeting the causal assumptions. For instance, we need to be clear about variables we need to adjust for to meet exchangeability. Additionally, while we can never confirm the assumptions of a causal method are met, but we can provide evidence through exploratory analysis (@sec-eda) or probe the consequences of violating them with sensitivity analysis (@sec-sensitivity).
...
A store serves precisely 100g of either chocolate or vanilla, both the same brand.
Thanks to this database, we have information on participants flavor choice and a variety of other other characteristics, including happiness (measured by the same tool we used above), with measurements at the door (baseline) and 30 minutes after eating their ice cream (follow-up).
In @tbl-target-trial, we lay out some ideas for a randomized trial of this question and the observational study that would emulate it.
...
|-----------------------------|----------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Eligibility criteria        | Who or what should be included in the study?                                                                               | Inclusion: Age 18 to 65. Exclusion: no lactose intolerance, no allergy to any ingredients, entered store within the week of the study.                                                                                                            | Same as target trial.                                                                                                                                                                                                                                                                                              |
| Exposure definition         | When eligible, what precise exposure will units under study receive?                                                       | 100g vanilla or chocolate ice cream in a bowl, both Ben and Jerzy brand ice cream.                                                                                                                                                                | Same as target trial.                                                                                                                                                                                                                                                                                              |
| Assignment procedures       | How will eligible units be assigned to an exposure?                                                                        | Participants are randomized with a 50% probability of either flavor. Assignment is non-blinded.                                                                                                                                                   | Participants are assigned the flavor consistent with their data, e.g., the flavor they chose. Randomization is emulated using baseline covariates.                                                                                                                                                                 |
| Follow-up period            | When does follow-up start and end?                                                                                         | Start: when eligibility criteria met and flavor assigned. End: 30 minutes after flavor assignment.                                                                                                                                                | Same as target trial.                                                                                                                                                                                                                                                                                              |
...
explain: we think the causes of flavor are ...
and the causes of happiness are ...
callout: ITT and per-protocol, weighting for rcts
