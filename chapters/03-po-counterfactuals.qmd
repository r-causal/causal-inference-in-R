...
plot_data |> 
  ...
  geom_dotplot(aes(fill = observed), alpha = .5, color = "grey50", stackratio = 1.5, dotsize = 1, show.legend = FALSE) + 
  stat_summary(
    fun = "mean", 
    size = 2.5, 
    geom = "point",
    position = position_nudge(y = 0.028)
  ) + 
  stat_summary(
    fun = "mean",
    geom = "text",
  ...
  geom_curve(
    data = avg_labels,
    aes(
      x = happiness + 1, xend = happiness - .1, y = 2.3, yend = 2.08
    ),
    curvature = 0.6,
    arrow = arrow(length = unit(0.02, "npc")),
    inherit.aes = FALSE,
  ...
  geom_curve(
    ...
    aes(x = x, xend = xend + .2, y = y + .1, yend = yend - .2),
    curvature = .1,
...
data_observed_exch |>
  ...
  stat_summary(
    ...
    size = 2.5, 
    geom = "point",
    position = position_nudge(y = 0.056)
  ) +
  stat_summary(
    fun = "mean",
    geom = "text",
...
#| code-fold: true
#| warning: false
# Explicitly calculate stacking positions
# because `geom_dotplot()` isn't working well with a y axis
# and color aes
stacked_data <- data_observed_struc |> 
  ...
  mutate(
    observed = if_else(exposure == potential_outcome, "observed", "unobserved"),
    potential_outcome = paste0("potential outcome: y(", potential_outcome, ")"),
    exposure = if_else(exposure == "vanilla", "actually\nate vanilla", "actually\nate chocolate")
  ) |> 
  drop_na() |> 
  group_by(exposure, potential_outcome, happiness) |> 
  mutate(
    # Assign stacking positions explicitly
    stack_position = row_number() - 1 
  ) |> 
  ungroup()

# Pre-calculate mean summaries
summary_data <- stacked_data |> 
  group_by(exposure, potential_outcome, observed) |> 
  summarize(
    mean_happiness = mean(happiness, na.rm = TRUE),
    .groups = "drop"
  ) |> 
  mutate(y_numeric = as.numeric(as.factor(exposure)))

ggplot(stacked_data, aes(x = happiness, y = exposure, fill = is_missing_y_vanilla, color = is_missing_y_vanilla, shape = observed)) +
  geom_point(
    data = stacked_data |> filter(observed == "observed"),
    aes(
      y = as.numeric(as.factor(exposure)) + stack_position * 0.1
    ),
    size = 3,
    shape = 21,
    alpha = 0.8
  ...
  geom_point(
    data = stacked_data |> filter(observed == "unobserved"),
    aes(
      x = happiness,
      y = as.numeric(as.factor(exposure)) + stack_position * 0.1
    ),
  ...
  geom_point(
    data = summary_data |> filter(observed == "observed"),
    aes(x = mean_happiness, y = y_numeric),
    size = 3,
    shape = 21,
    fill = "grey30",
    color = "grey30",
  ...
  geom_point(
    data = summary_data |> filter(observed == "unobserved"),
    aes(x = mean_happiness, y = y_numeric),
    size = 3,
    shape = 21,
    fill = "white",
  ...
  geom_text(
    data = summary_data,
    aes(x = mean_happiness, y = y_numeric, label = round(mean_happiness, 1)),
    vjust = 2,
    inherit.aes = FALSE
  ) +
  facet_wrap(~ potential_outcome) +
  scale_y_continuous(
    breaks = 1:2,
    labels = c("actually ate\nchocolate", "actually ate\nvanilla"),
    limits = c(.5, 2.5)
  ) +
  theme(
    panel.grid.major.y = element_blank(),
