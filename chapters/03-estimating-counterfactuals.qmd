...
In reality, we cannot observe both potential outcomes at any given moment; each individual in our study can only eat one flavor of ice cream at the moment of study[^03-estimating-counterfactuals-1].
...
[^03-estimating-counterfactuals-1]: Ice cream swirls were prohibited per the study protocol.
...
These methods all assume[^03-estimating-counterfactuals-2] three things: exchangeability, positivity, and consistency.
...
[^03-estimating-counterfactuals-2]: These *causal* assumptions are in addition to any *statistical* assumptions, such as distributional assumptions, that the estimators we use require.
...
We've exchanged the groups by flipping their assignments, but we can still detect the right causal effect [^03-estimating-counterfactuals-3].

[^03-estimating-counterfactuals-3]: The numbers are slightly different because of the sample size.
...
## When do standard methods estimate causal effects?
