...
It's easy to imagine a randomized trial that deals with this question: participants in a study are randomly assigned to use a bed net (Group A) or not (Group B), and we follow them over time to see if there is a difference in malaria risk between groups.
...
-   **Risk among whom?**
    What is the population to whom we're trying to apply this knowledge?
...
#| echo = FALSE
means <- net_data |>
  summarize(malaria_risk = mean(malaria_risk), .by = net) |>
  pull(malaria_risk)
```
...
If we attempt to interpret the above simple estimates as-is, a problem that we face is that other factors may be responsible for the effect we're seeing.
...
Importantly, DAG construction is not a data-driven approach; rather, we arrive at our presumed DAG through expert background knowledge regarding the exposure and outcome in question.
...
If this DAG is correct, we have *eight* causal pathways which will present themselves in the data as associations between bed net use and malaria risk.
Specifically, the DAG reveals a path between net usage and malaria risk alongside seven other *confounding* pathways.
...
We'll use the {[rsample](https://rsample.tidymodels.org/)} package from the tidymodels ecosystem to work with bootstrap samples.
...
The {[tipr](https://github.com/r-causal/tipr)} package is a toolkit for conducting sensitivity analyses.
