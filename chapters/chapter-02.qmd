...
Randomization is often the best way to estimate a causal effect of an intervention because it reduces the number of assumptions we need to make for that estimate to be valid (we will discuss these assumptions @sec-assump).
...
-   Malaria risk is causally impacted by net usage, income, health, temperature, and insecticide resistance. Insecticide resistance, however, does not affect our exposure (net usage), so we omit it from the DAG.
...
We might also want to know how well-balanced the groups are by confounders.
...
mosquito_dag_full <- dagify(
  malaria_risk ~ net + income + health + temperature + genetic_resistance,
