...
These techniques are known as *sensitivity analyses*: How sensitive is our result to conditions other than those laid out in our assumptions and analysis?
...
Let's say, though, that we had used @fig-dag-magic-missing instead, which is missing an arrow from the park close to whether there was an extra magic morning.
...
#|   in the morning at a particular park and the average wait
#|   time between 9 am and 10 am.
#|   This DAG has no arrow from park close time to Extra Magic Hours.
...
Some slight variation in the estimates is expected since they are estimated using different variables; if this DAG were right, we should see them much more closely aligned than this.
...
Let's call today *i* and the previous day *i - n*, where *n* is the number of days before the negative exposure.
...
From a decision point of view, though, the difference is slight in absolute terms and results in an effect in the same direction.
