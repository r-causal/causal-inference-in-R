...
The same mathematical underpinnings of DAGs that allow us to to query them for things like adjustment sets also allow us to query other implications of DAGs. One of the simplest is that, if your DAG is right and your data are well-measured, any valid adjustment set should result in an unbiased estimate of the causal effect. Let's consider the DAG we introduced in @fig-dag-magic
...
In @fig-dag-magic-orig, there's only one adjustment set because all three confounders represent independent backdoor paths. Let's say, though, that we had used @fig-dag-magic-missing instead, which is missing an arrow from park close time to whether or not there was an extra magic morning.
...
This introduces two new relationships that should be independent. In @fig-conditional-ind-misspec, we see an additional association between ticket season and Extra Magic Morning.
