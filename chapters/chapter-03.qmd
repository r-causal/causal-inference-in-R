...
data |>
  knitr::kable(
    col.names = c("id", "$$P_i(\\textrm{chocolate})$$", "$$P_i(\\textrm{vanilla})$$", "$$P_i(\\textrm{chocolate}) - P_i(\\textrm{vanilla})$$")) |>
...
We know the *true* average causal effect in the sample is `r mean(data |> pull(causal_effect))`, however our estimated causal effect (because our data are not consistent with the question we are asking) is `r data_observed |> group_by(exposure) |> summarise(avg_outcome = mean(observed_outcome)) |> mutate(exposure = c(1, 0)) |> pivot_wider(names_from = exposure, values_from = avg_outcome, names_prefix = "x_") |> summarise(estimate = x_1 - x_0) |> pull() |> round(1)`. This demonstrates what can go wrong when *consistency* is violated.
