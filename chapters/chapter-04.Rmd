...
As defined in the previous chapter, <!--FIXME make sure that this is defined in the previous chapter --> an exposure in the context of this book is often a modifiable event or condition that occurs prior to the outcome.
...
*Draft:* looking for lack of overlap as a potential positivity problem.
But too much overlap may be indicative of a poor model(? not necessarily. depends on the relative covariate distributions in the two groups).
Avg treatment effect among treated easier to estimate with precision (because of higher counts) than the control group.
A single outlier in either group with respect to range could be a problem and warrant data inspection (do this here?) *look at the model coefs here*.
When do we talk about trimming / matching?
This may be a "nice" example -- should we also show a bad model (maybe only with ticket season or possibly overspecifying it)
...
ggplot() +
  ...
  labs(
    ...
    <b style='color:orange'>exposed</b> and <b style='color:#6495ED'>unexposed</b>"
  ) + 
...
If the mean is far from 1, we may have issues with model mispecification or violations of positivity [@hernán2021].
