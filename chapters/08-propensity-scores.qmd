...
There are many ways to estimate the propensity score; for binary exposures, people typically use logistic regression, although other approaches are used, as well.
...
using the `augment()` function from the [{`broom`}](https://broom.tidymodels.org/) package, we can extract these propensity scores and add them to our original data frame, so we'll use that approach.
By default, the predictions will be on the *linear logit* scale.
...
It's useful to visualize the distribution of propensity scores by exposure group.
