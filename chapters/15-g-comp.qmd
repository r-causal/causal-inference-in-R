...
# fits is a list which is returned from our fit_models function
simulate_process <- function(.data, n_sample = 10000, fits) {
  ...
  df_sim_time_1 <- fits$fit_extra_magic |> 
  ...
  df_sim_time_2 <- df_sim_time_1 |> 
    mutate(
      wait_minutes_posted_avg = 
        c(rep(1, n_sample/2), rep(0, n_sample/2))
    )
  
  # Simulate the outcome 
  df_outcome <- fits$fit_wait_minutes_actual |> 
...
```{r}
compute_contrast <- function(.data) {
...
test_fits <- fit_models(wait_times)
simulate_process(wait_times, n_sample = 10000, test_fits)
```
