...
mosquito_dag |>
  ...
  ggplot(aes(x = x, y = y, xend = xend, yend = yend, col = effects, alpha = path)) +
  geom_dag_edges(aes(edge_alpha = path, edge_colour = effects), show.legend = FALSE) +
  geom_dag_point(
    data = function(.x) dplyr::filter(.x, is.na(path)), key_glyph = glyph
  ) +
  geom_dag_point(
    data = function(.x) dplyr::filter(.x, !is.na(path)), key_glyph = glyph
  ) +
  facet_wrap(vars(fct_inorder(factor(set), ordered = TRUE))) +
  expand_plot(
...
In other words, this household will be up-weighted almost 3 times compared to the naive linear model we fit above.
...
Unfortunately, the confidence intervals we're using are wrong because they don't account for the dependence in the weights!
...
The nominal coverage of the confidence intervals will thus be wrong (they aren't actually 95% CIs) and may lead to misinterpretation.

We've got several ways we can address this problem, which we'll discuss in detail in Chapter \@ref(TODO), including the bootstrap, robust standard errors, and manually accounting for the dependence with empirical sandwich estimators.
For this example, we'll use the bootstrap, a flexible tool that calculates distributions of parameters using re-sampling.
To work with bootstrap samples, we'll use the rsample package from the tidymodels ecosystem.

::: callout-note
## The Bootstrap

The bootstrap is a simple but flexible algorithm for calculating statistics using re-sampling with replacement.
It's particularly useful when a closed-form solution doesn't exist to calculate something, as is commonly the case in causal inference (particularly for standard errors), as well as when we suspect the parametric calculations are not valid for a given situation.
...
We'll use rsample, a more modern alternative for re-sampling, throughout the book, but generally we start with writing a function to calculate the estimate we're interested in.
...
We then end up with a distribution of `estimate`s, with which we can calculate population statistics with, such as point estimates, standard errors, and confidence intervals.
:::
...
This still won't give us the correct confidence intervals, though, because it treats the inverse probability weights as fixed values.
...
That means, for every bootstrap sample, we need to fit the propensity score model, calculate the inverse probability weights, then fit the weighted outcome model.
...
Now that we know exactly how we're going to calculate the estimate for each iteration, let's create the bootstrapped dataset with rsample's `bootstraps()` function.
...
bootstrapped_net_data <- bootstraps(
  net_data, 
  times = 1000, 
  # required to later calculate CIs
  apparent = TRUE
)
...
At it's heart, the calculation we're doing is
...
The result is another nested data frame, this time with a new column, `boot_fits`.
...
ipw_results |> 
  mutate(
    ...
    \(.fit) .fit |> filter(term == "netTRUE") |> pull(estimate)
    )
  ) |> 
  ggplot(aes(estimate)) +
...
This gives a sense of the variation in `estimate`, but let's calculate 95% confidence intervals from the bootstrapped distribution using rsample's `int_t()` :
...
But what if we're more on the less right side?
...
They're a lot of potential sources of bias in any study, and many sensitivity analyses to go along with them; we'll focus on the assumption of no counfounding.

Let's start with a broad sensitivity analysis, then we'll ask questions about specific unmeasured confounders.
When we have less information about unmeausured confounders, we can use tipping point analysis to ask the question: how much confounding would it take to tip my estimate to the null?
In other words, what would the strength of the unmeasured confounder have to be to completely explain away our results?
The tipr package is a toolkit for conducting sensitivity analyses.
Let's examine the tipping point for an unknown, normally-distrubted confounder.
The `tip_coef()` function takes an estimate (a beta coefficient from a regression model) and the either 1) the scaled differences in means of the confounder between exposure groups or 2) the effect of the confounder on the outcome.
...
```{r tip-coef-net}
#| fig.cap: "A tipping point analaysis under several counfounding scenarios where the unmeasured confounder is a normally-distrubuted continuous variable. The line represents the strength of counfounding necessary to tip the upper confidence interval of the causal effect estimate to 0. The x-axis represents the coefficient of the confounder-outcome relationship adjusted for the exposure and the set of measured confounders. The y-axis represents the scaled mean difference of the confounder between exposure groups."
library(tipr)
tipping_points <- tip_coef(estimates$conf.high, exposure_confounder_effect = 1:5)
...
If we had an unmeasured confounder where the average difference between exposure groups was 1, the confounder would need to decrease malaria risk by about `r round(tipping_points$exposure_confounder_effect[[1]], digits = 1)`.
...
Conversely, if the relationship between net use and the unmeasured confounder is very strong, with a mean scaled difference of 5, the confounder-malaria relationship only needs to be `r round(tipping_points$exposure_confounder_effect[[5]], digits = 1)` .
...
There are some ethnic groups, such as the Fulani, who have a genetic resistance to malaria [@arama2015].
Let's say that in our simulated data, there is an unnamed ethnic group who shares this genetic resistance to malaria.
For historic reasons, bed net use in this fictional group is also very high.
...
2.  About 25% of people who use nets in our study have this genetic resistance.
3.  About 5% of people who don't use nets have this genetic resistance. <!--TODO: Need to circle back to this after tipr supports linear coefs with binary exp--> With this amount of information, we can use tipr to adjust the estimates we calculated for the unmeasured confounder. We'll use `adjust_coef_with_binary()` to calculate the adjusted estimates.

```{r}
adjusted_estimates <- adjust_coef_with_binary(
  unlist(estimates), 
...
The unmeasured confounder is available in the dataset `net_data_full` as `genetic_resistance`.
If we recalculate the IPW estimate of the average treatment effect of nets on malaria risk, we get `r est_ci(boot_estimate_full, rsample = TRUE)`, much closer to the true answer of -10.

What do you think?
Is this estimate reliable?
Did we do a good job addressing the assumptions we need to make for a causal effect, particularly that there is no confounding?
How might you criticize this model, and what would you do differently?
Ok, we know that -10 is the right answer because the data are simulated, but in practice, we can never be sure, so we need need to continue probing our assumptions until we're confident that they are robust.
<!-- TODO: Maybe use sickle cell as example of precision variable in the variable selection section later in the book. Interesting example because it's not possible for sickle cell to be downstream. Consider in context of overadjustment. -->
