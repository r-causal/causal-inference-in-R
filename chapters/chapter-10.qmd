...
Suppose we have a blue button on our website and we are interested in assessing whether changing the color to blue increases the average number of items a user purchases.
We can create an A/B test where we randomly assign a subset of users to see the new greed button (group A) or the blue button (group B).
...
The true average treatment effect is 1, implying that on average seeing the green button increases the number of items purchased by 1.
...
d_wts <- glm(x ~ z, data = d, family = binomial("probit")) |>
  augment(newdata = d, type.predict = "response") |>
...
## Examine 5 different sample sizes, simulate each 100 times
set.seed(1)
o <- map_df(rep(c(50, 100, 500, 1000, 5000, 10000), 
                each = 1000), sim)
...
This is an example of finite sample bias. Notice here, even when the sample size is quite large (5,000) we still see some bias away from the "true" effect of 0. It isn't until a sample size of 10,000 that we start to see this bias disappearing.
...
#| fig-cap: "Finite sample bias with ATE weights () and ATO weights () created using correctly specified propensity score model, varying the sample size from n = 50 to n = 10,000"
