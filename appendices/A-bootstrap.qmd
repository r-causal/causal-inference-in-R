...
```{r}
#| label: fig-r-10
#| fig-cap: "The bootstrapped distributions of the coefficients from the model `lm(y ~ x + z, data = .df)`. The distributions were calculated with 10 bootstrapped samples."
unnested_results |> 
  ggplot(aes(estimate)) + 
...
```{r}
#| label: fig-r-1000
#| fig-cap: "The bootstrapped distributions of the coefficients from the model `lm(y ~ x + z, data = .df)`. The distributions were calculated with 1000 bootstrapped samples."
bootstrapped_samples_1k <- bootstraps(
  sampled_data, 
  times = 1000
) |> 
  mutate(
...
We'll also increase the number of samples to approximate the coverage better.
...
Allowing each observation to be independent in sampling also lets it represent others like it in the original population, much like when we up-weight samples in inverse probability weighting (see [Chapter -@sec-using-ps]).

## How many bootstrapped samples?

In this book, we use a thousand bootstrapped samples for most problems to balance stability and computational speed.
How many should you use in your real analysis?

You'll often see older recommendations in the tens or hundreds of samples, but these are from a time when processing power was more limited.
On modern computers (even personal laptops), it's practical to do many more.
@Hesterberg2015 suggests a thousand samples for rough estimates and ten to fifteen thousand samples for when accuracy matters.
By "accuracy," we mean minimal variance due to the bootstrap simulation itself.
A practical test for this is to try out `R` samples more than once and increase `R` until you are satisfied with the degree of stability in your results.
